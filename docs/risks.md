...
| **Engine math is wrong** → the whole product is worthless. | **High** | *Standing gate, in CI:* golden fixtures + dual reference-oracle cross-check + property tests + determinism, run in release config (`engine-gate.yml`, weekly + on engine paths). The current local release gate passes 92 tests. |
