...
3. Play: choose among five short opponent titles in a stationary vertical list. Tapping one opens a bottom sheet with two separate starting-habit scales; expand the technical numbers only if wanted. Back, Escape and the backdrop return to the same row. Start enters the fixed four-seat hand; step forward/back, then inspect the factual review and optional post-hand opponent cards. Its seats are Computer 1/2/3, but the chosen style only changes Computer 1's label, not the scripted actions.
