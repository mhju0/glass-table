...
Object.assign(copy.ko, {
  ...
  opponentsTitle:"연습 상대 고르기", opponentLead:"상대를 고르면 두 가지 습관을 볼 수 있어요.", opponentScaleTitle:"시작할 때의 습관", opponentScaleNote:"공용 카드가 나오기 전의 기준이에요. 자리와 상황에 따라 실제 행동은 달라져요.", enterScale:"판에 들어오기", raiseScale:"금액 올리기", scaleStart:"드물게", scaleEnd:"자주", opponentTechnical:"VPIP와 PFR은 기존 상대 모델에서 공용 카드가 나오기 전의 기준값이에요. 막대는 전체 시작 판의 0~100%를 나타내요. 실제 사람의 기록이나 네 자리 테이블의 관측값은 아니에요.", computer:"컴퓨터",
...
const state = { lang:systemLanguage(), langOption:"system", theme:"system", route:"learn", learnRecommendation:"next", settings:false, opp:"tag", oppDetails:false, chartChoice:null, chartExpanded:false, chartCell:"J5o", practiceStage:"idle", introReturnStage:"idle", q:0, answers:[], assisted:[], answer:null, help:false, tableIndex:-1, tableReview:false, hindsight:false, recordsState:"sample", timing:false, styleState:"sample", styleDetail:2 };
...
function opponentPicker(){
  ...
  return `<section class="page"><p class="eyebrow">${$("preview")} · ${$("navPlay")}</p><h1 class="title">${$("opponentsTitle")}</h1><p class="sub">${$("opponentLead")}</p>
    <div class="section opponent-grid stack">${opponents.map(o=>`<button type="button" class="opponent ${o.id===state.opp?"active":""}" data-action="opp-select" data-value="${o.id}" aria-pressed="${o.id===state.opp}" aria-controls="opponent-habits"><span class="name">${state.lang==="ko"?o.ko:o.en}</span><span class="desc">${state.lang==="ko"?o.detailKo:o.detailEn}</span></button>${o.id===state.opp?`<section id="opponent-habits" class="raised behavior-panel"><h2>${$("opponentScaleTitle")}</h2>${behaviorScale($("enterScale"),selected.vpip)}${behaviorScale($("raiseScale"),selected.pfr)}<div class="behavior-ends"><span>${$("scaleStart")}</span><span>${$("scaleEnd")}</span></div><p class="tiny">${$("opponentScaleNote")}</p></section>`:""}`).join("")}</div>
    <div class="section button-row">${btn($("details"),"opp-details")}${btn($("pickOpponent"),"table-start","primary")}</div>${state.oppDetails?`<div class="detail-panel"><h2>${state.lang==="ko"?selected.ko:selected.en} (${selected.name})</h2><p>${$("opponentTechnical")}</p>${labelValue($("vpip"),`${selected.vpip}%`)}${labelValue($("pfr"),`${selected.pfr}%`)}</div>`:""}</section>`;
}
...
function render(){const y=window.scrollY;document.documentElement.lang=state.lang;document.documentElement.dataset.theme=state.theme;let content=state.route==="learn"?learn():state.route==="chart"?chart():state.route==="opponents"?opponentPicker():state.route==="table"?(state.tableReview?tableReview():table()):state.route==="records"?records():state.practiceStage==="intro"?intro():state.practiceStage==="summary"?practiceSummary():question();document.getElementById("app").innerHTML=`<div class="shell"><header class="topbar" ${state.settings?"inert":""}><span class="brand">Glass Table</span><div class="top-actions"><button type="button" class="icon-button small-text" data-action="settings-open" aria-label="${$("settings")}">${$("settings")}</button></div></header><main id="main" ${state.settings?"inert":""}>${content}</main><nav class="bottomnav" ${state.settings?"inert":""} aria-label="${state.lang==="ko"?"주요 화면":"Main screens"}">${[["learn","♠","navLearn"],["opponents","▤","navPlay"],["records","▥","navProgress"]].map(([v,mark,label])=>{const active=v==="learn"?["learn","chart","practice"].includes(state.route):v==="opponents"?["opponents","table"].includes(state.route):state.route==="records";return `<button type="button" data-action="nav" data-value="${v}" class="${active?"active":""}" ${active?'aria-current="page"':""}><span class="navmark" aria-hidden="true">${mark}</span>${$(label)}</button>`}).join("")}</nav>${state.settings?settings():""}</div>`;window.scrollTo(0,y);}
