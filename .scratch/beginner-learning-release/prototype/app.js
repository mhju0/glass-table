...
const copy = {
  ko: {
    ...
    navLearn:"배우기", navTable:"테이블", navRecords:"기록",
    ...
    timingShow:"답변 시간 보기", timingHide:"답변 시간 감추기", assisted:"도움을 보고 푼 문제", independent:"혼자 푼 문제", chartAria:"13줄 13칸 카드 표", resume:"풀던 문제 계속하기", ownCell:"내 J5o 칸으로 이동", menuHelp:"도움말", introReturn:"연습으로 돌아가기"
  },
  en: {
    ...
    navLearn:"Learn", navTable:"Table", navRecords:"Records",
    ...
    timingShow:"Show answer time", timingHide:"Hide answer time", assisted:"Questions answered with help", independent:"Questions answered independently", chartAria:"13-row, 13-column card chart", resume:"Continue the round", ownCell:"Go to my J5o square", menuHelp:"Help", introReturn:"Return to practice"
  }
};

Object.assign(copy.ko, {
  navPlay:"플레이", navProgress:"기록", navTable:"플레이", learnRecommended:"다음에 해 볼 일", learnPreviewState:"추천 카드 예시", recommendNext:"다음 배우기", recommendReview:"복습 알림", recommendIntro:"읽던 소개", recommendResume:"풀던 문제", recommendDone:"이번 연습을 마쳤어요", recommendNextBody:"쇼다운에서 카드 다섯 장을 비교하는 연습부터 시작해요.", recommendReviewBody:"쇼다운을 다시 풀어 보는 복습 카드 예시예요. 실제 복습 일정과 연결되지 않아요.", recommendIntroBody:"읽던 소개로 돌아가요.", recommendResumeBody:"풀던 다섯 문제를 이어서 해요. 이미 고른 답은 그대로예요.", recommendDoneBody:"연습 결과를 확인해요.", recommendNextButton:"쇼다운 배우기", recommendReviewButton:"쇼다운 복습하기", recommendIntroButton:"소개 이어 보기", recommendDoneButton:"연습 결과 보기", sampleSkills:"화면 예시", sampleSkillsNote:"아래 두 화면만 연결된 시안이에요. 전체 학습 길은 아직 이 시안에 없어요.",
  opponentsTitle:"연습 상대 고르기", opponentLead:"상대를 고르면 두 가지 습관을 볼 수 있어요.", opponentScaleTitle:"시작할 때의 습관", opponentScaleNote:"공용 카드가 나오기 전의 기준이에요. 자리와 상황에 따라 실제 행동은 달라져요.", enterScale:"판에 들어오기", raiseScale:"금액 올리기", scaleStart:"드물게", scaleEnd:"자주", opponentTechnical:"VPIP와 PFR은 기존 상대 모델에서 공용 카드가 나오기 전의 기준값이에요. 막대는 전체 시작 판의 0~100%를 나타내요. 실제 사람의 기록이나 네 자리 테이블의 관측값은 아니에요.", computer:"컴퓨터", selectedOpponent:"선택됨", backToOpponents:"뒤로", backToOpponentsA11y:"상대 목록으로", opponentDetails:"규칙 숫자 보기",
  ...
  notEngine:"이 화면은 고정된 한 판입니다. 고른 상대의 설명만 컴퓨터 1 자리에 표시하고, 행동은 바뀌지 않아요.",
...
Object.assign(copy.en, {
  navPlay:"Play", navProgress:"Progress", navTable:"Play", navRecords:"Progress", learnRecommended:"Suggested next", learnPreviewState:"Recommendation examples", recommendNext:"Next lesson", recommendReview:"Review reminder", recommendIntro:"Introduction in progress", recommendResume:"Round in progress", recommendDone:"Round finished", recommendNextBody:"Begin by comparing the best five cards in Showdown.", recommendReviewBody:"A sample reminder to revisit Showdown. It is not connected to a real review schedule.", recommendIntroBody:"Return to the introduction you were reading.", recommendResumeBody:"Continue the five questions you started. Your answers are still here.", recommendDoneBody:"See your result.", recommendNextButton:"Learn Showdown", recommendReviewButton:"Review Showdown", recommendIntroButton:"Continue introduction", recommendDoneButton:"See round summary", sampleSkills:"Screen examples", sampleSkillsNote:"Only these two sample skills are connected here. The full learning path is not in this mock.",
  opponentsTitle:"Choose an opponent", opponentLead:"Choose a style to see two habits.", opponentScaleTitle:"Starting habits", opponentScaleNote:"Before shared cards appear. Seat and situation affect actual choices.", enterScale:"Joins a hand", raiseScale:"Raises the bet", scaleStart:"Rarely", scaleEnd:"Often", opponentTechnical:"VPIP and PFR are starting-hand settings from the existing opponent model. Each bar covers 0–100% of starting hands. They are not a person's history or measured results at this four-seat table.", computer:"Computer", selectedOpponent:"Selected", backToOpponents:"Back", backToOpponentsA11y:"Back to opponents", opponentDetails:"See rule numbers",
  ...
  notEngine:"This is one fixed hand. Your chosen style appears at Computer 1, but scripted actions do not change.",
...
const opponents = [
  { id:"nit", ko:"신중형", en:"Cautious", name:"Nit", detailKo:"좋은 시작 카드를 기다려요.", detailEn:"Waits for strong starting cards.", vpip:12, pfr:9 },
  { id:"tag", ko:"선별형", en:"Selective", name:"TAG", detailKo:"고른 카드로 들어가면 대개 올려요.", detailEn:"Chooses fewer hands, then usually raises.", vpip:20, pfr:17 },
  { id:"lag", ko:"공격형", en:"Aggressive", name:"LAG", detailKo:"더 여러 판에 들어가고, 대개 올려요.", detailEn:"Joins more hands and usually raises.", vpip:27, pfr:22 },
  { id:"station", ko:"콜 위주형", en:"Caller", name:"Calling station", detailKo:"여러 판에 들어가고, 올리기보다 맞춰요.", detailEn:"Joins many hands and usually matches instead of raising.", vpip:40, pfr:10 },
  { id:"maniac", ko:"매우 공격형", en:"Very aggressive", name:"Maniac", detailKo:"아주 여러 판에 들어가고 자주 올려요.", detailEn:"Joins very many hands and often raises.", vpip:55, pfr:40 }
];
...
const state = { lang:systemLanguage(), langOption:"system", theme:"system", route:"learn", learnRecommendation:"next", settings:false, opp:"tag", oppSheet:false, chartChoice:null, chartExpanded:false, chartCell:"J5o", practiceStage:"idle", introReturnStage:"idle", q:0, answers:[], assisted:[], answer:null, help:false, tableIndex:-1, tableReview:false, hindsight:false, recordsState:"sample", timing:false, styleState:"sample", styleDetail:2 };
...
function learn() {
  const pending=state.practiceStage==="question";
  const done=state.practiceStage==="summary";
  const kind=state.practiceStage==="intro"?"intro":pending?"resume":done?"done":state.learnRecommendation;
  const recommendation={
    intro:[$("recommendIntro"),$("recommendIntroBody"),$("recommendIntroButton"),"practice-intro"],
    resume:[$("recommendResume"),$("recommendResumeBody"),$("resume"),"practice-resume"],
    done:[$("recommendDone"),$("recommendDoneBody"),$("recommendDoneButton"),"practice-resume"],
    review:[$("recommendReview"),$("recommendReviewBody"),$("recommendReviewButton"),"practice-intro"],
    next:[$("recommendNext"),$("recommendNextBody"),$("recommendNextButton"),"practice-intro"]
  }[kind];
  return `<section class="page"><p class="eyebrow">${$("preview")} · ${$("navLearn")}</p><h1 class="title">${$("learnTitle")}</h1><p class="sub">${$("learnSub")}</p>
    <section class="section raised recommendation"><p class="eyebrow">${$("learnRecommended")}</p><h2>${recommendation[0]}</h2><p class="muted">${recommendation[1]}</p><div class="section">${btn(recommendation[2],recommendation[3],"primary block")}</div></section>
    <div class="section sample-switch"><span class="tiny">${$("learnPreviewState")}</span>${btn($("recommendNext"),"learn-sample",state.learnRecommendation==="next"?"selected":"",'data-value="next" aria-pressed="'+(state.learnRecommendation==="next")+'"')}${btn($("recommendReview"),"learn-sample",state.learnRecommendation==="review"?"selected":"",'data-value="review" aria-pressed="'+(state.learnRecommendation==="review")+'"')}</div>
    <section class="section"><h2>${$("sampleSkills")}</h2><p class="tiny">${$("sampleSkillsNote")}</p><div class="sample-skills"><div class="surface"><h3>${$("learnIntro")}</h3><p class="muted">${$("learnIntroSub")}</p><div class="section">${btn($("goIntro"),"practice-intro","block")}</div></div><div class="surface"><h3>${$("learnChart")}</h3><p class="muted">${$("learnChartSub")}</p><div class="section">${btn($("goChart"),"chart-open","block")}</div></div></div></section></section>`;
}
...
const behaviorScale = (label,value) => `<div class="behavior-row"><div class="behavior-label"><strong>${label}</strong></div><div class="behavior-track" role="img" aria-label="${label}: ${value}%, ${$("scaleStart")}–${$("scaleEnd")}"><span class="behavior-fill" style="width:${value}%"></span><span class="behavior-dot" style="left:${value}%"></span></div></div>`;
function opponentPicker(){
  return `<section class="page"><p class="eyebrow">${$("preview")} · ${$("navPlay")}</p><h1 class="title">${$("opponentsTitle")}</h1><p class="sub">${$("opponentLead")}</p>
    <div class="section opponent-grid stack">${opponents.map(o=>`<button type="button" class="opponent ${o.id===state.opp?"active":""}" data-action="opp-select" data-value="${o.id}" aria-label="${state.lang==="ko"?o.ko:o.en}. ${state.lang==="ko"?o.detailKo:o.detailEn}${o.id===state.opp?` ${$("selectedOpponent")}`:""}" aria-pressed="${o.id===state.opp}" aria-haspopup="dialog"><span class="name">${state.lang==="ko"?o.ko:o.en}</span><span class="selected-mark" aria-hidden="true">✓</span><span class="desc">${state.lang==="ko"?o.detailKo:o.detailEn}</span></button>`).join("")}</div></section>`;
}
function opponentSheet(){
  const selected=opponents.find(o=>o.id===state.opp);
  return `<div class="overlay" data-action="opp-backdrop"><section class="sheet opp-sheet" role="dialog" aria-modal="true" aria-labelledby="opponent-title" aria-describedby="opponent-description"><div class="sheet-head"><h2 id="opponent-title">${state.lang==="ko"?selected.ko:selected.en}</h2>${btn($("backToOpponents"),"opp-close","",`aria-label="${$("backToOpponentsA11y")}"`)}</div><p id="opponent-description" class="muted">${state.lang==="ko"?selected.detailKo:selected.detailEn}</p><div class="section behavior-panel"><h3>${$("opponentScaleTitle")}</h3>${behaviorScale($("enterScale"),selected.vpip)}${behaviorScale($("raiseScale"),selected.pfr)}<div class="behavior-ends"><span>${$("scaleStart")}</span><span>${$("scaleEnd")}</span></div><p class="tiny">${$("opponentScaleNote")}</p></div><details class="opp-disclosure section"><summary>${$("opponentDetails")}</summary><div class="detail-panel"><h3>${selected.name}</h3><p>${$("opponentTechnical")}</p>${labelValue($("vpip"),`${selected.vpip}%`)}${labelValue($("pfr"),`${selected.pfr}%`)}</div></details><div class="section">${btn($("pickOpponent"),"table-start","primary block")}</div></section></div>`;
}
const seatName = id => id==="you"?$("you"):`${$("computer")} ${ {a:1,b:2,c:3}[id]}`;
...
function render(){const y=window.scrollY;document.documentElement.lang=state.lang;document.documentElement.dataset.theme=state.theme;const modal=state.settings||state.oppSheet;let content=state.route==="learn"?learn():state.route==="chart"?chart():state.route==="opponents"?opponentPicker():state.route==="table"?(state.tableReview?tableReview():table()):state.route==="records"?records():state.practiceStage==="intro"?intro():state.practiceStage==="summary"?practiceSummary():question();document.getElementById("app").innerHTML=`<div class="shell"><header class="topbar" ${modal?"inert":""}><span class="brand">Glass Table</span><div class="top-actions"><button type="button" class="icon-button small-text" data-action="settings-open" aria-label="${$("settings")}">${$("settings")}</button></div></header><main id="main" ${modal?"inert":""}>${content}</main><nav class="bottomnav" ${modal?"inert":""} aria-label="${state.lang==="ko"?"주요 화면":"Main screens"}">${[["learn","♠","navLearn"],["opponents","▤","navPlay"],["records","▥","navProgress"]].map(([v,mark,label])=>{const active=v==="learn"?["learn","chart","practice"].includes(state.route):v==="opponents"?["opponents","table"].includes(state.route):state.route==="records";return `<button type="button" data-action="nav" data-value="${v}" class="${active?"active":""}" ${active?'aria-current="page"':""}><span class="navmark" aria-hidden="true">${mark}</span>${$(label)}</button>`}).join("")}</nav>${state.settings?settings():state.oppSheet?opponentSheet():""}</div>`;document.body.style.overflow=modal?"hidden":"";window.scrollTo(0,y);}
function route(to){state.route=to;window.scrollTo(0,0);}
document.addEventListener("click", e=>{
  const target=e.target.closest("[data-action]");
  if(!target)return;
  const a=target.dataset.action,v=target.dataset.value;
  if((a==="settings-backdrop"||a==="opp-backdrop")&&e.target!==target)return;
  switch(a){
    case"settings-open":state.oppSheet=false;state.settings=true;break;
    ...
    case"learn-sample":state.learnRecommendation=v;break;
    ...
    case"opp-select":state.opp=v;state.oppSheet=true;break;
    case"opp-close":case"opp-backdrop":state.oppSheet=false;break;
    case"table-start":state.oppSheet=false;state.tableIndex=-1;state.tableReview=false;state.hindsight=false;route("table");break;
  ...
  else if(a==="opp-select")document.querySelector('[data-action="opp-close"]')?.focus();
  else if(a==="opp-close"||a==="opp-backdrop")document.querySelector(`[data-action="opp-select"][data-value="${state.opp}"]`)?.focus({preventScroll:true});
  else if(a==="table-start"){const heading=document.querySelector('main h1');heading?.setAttribute('tabindex','-1');heading?.focus({preventScroll:true});}
...
document.addEventListener("keydown",e=>{
  if(e.key==="Escape"&&(state.settings||state.oppSheet)){
    e.preventDefault();
    const wasSettings=state.settings;
    state.settings=false;state.oppSheet=false;render();
    document.querySelector(wasSettings?'[data-action="settings-open"]':`[data-action="opp-select"][data-value="${state.opp}"]`)?.focus({preventScroll:true});
  }
  if(e.key==="Tab"&&(state.settings||state.oppSheet)){
    const controls=[...document.querySelectorAll('.sheet button:not(:disabled),.sheet summary')];
